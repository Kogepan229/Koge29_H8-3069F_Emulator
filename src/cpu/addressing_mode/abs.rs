...
impl Cpu {
    ...
    async fn _write_abs16_w(&mut self, mut addr: u16, value: u16) -> Result<()> {
        ...
        if addr & 0x8000 == 0x0000 {
            self.bus
                .lock()
                .await
                .write(addr as u32, (value >> 8) as u8)?;
            self.bus
                .lock()
                .await
                .write((addr + 1) as u32, value as u8)?;
        } else {
            self.bus
                .lock()
                .await
    ...
    async fn _read_abs16_w(&self, mut addr: u16) -> Result<u16> {
        ...
        if addr & 0x8000 == 0x0000 {
            return Ok((self.bus.lock().await.read(addr as u32)? as u16) << 8
                | self.bus.lock().await.read((addr + 1) as u32)? as u16);
        } else {
            return Ok(
                (self.bus.lock().await.read(0xff0000 | addr as u32)? as u16) << 8
                    | self.bus.lock().await.read((0xff0000 | addr as u32) + 1)? as u16,
            );
        }
    }
...
mod tests {
    ...
    async fn test_write_abs8_w() {
        ...
        assert_eq!(
            (cpu.bus.lock().await.read(0xffff10).unwrap() as u16) << 8
                | cpu.bus.lock().await.read(0xffff11).unwrap() as u16,
            0x0fff
        );

        cpu.write_abs8_w(0x01, 0x0fff).await.unwrap();
        assert_eq!(
            (cpu.bus.lock().await.read(0xffff00).unwrap() as u16) << 8
                | cpu.bus.lock().await.read(0xffff01).unwrap() as u16,
            0x0fff
        );
    }
