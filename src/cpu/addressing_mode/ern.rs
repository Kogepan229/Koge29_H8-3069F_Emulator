...
impl<'a> Cpu<'a> {
    pub(in super::super) fn write_ern_b(
        &mut self,
        register_field: u8,
        value: u8,
    ...
    pub(in super::super) fn read_ern_b(&self, register_field: u8) -> Result<u8, AddressingError> {
        let addr = self.read_rn_l(register_field)?;
        Ok(self.read_abs24_b(addr & 0x00ffffff))
    }

    pub(in super::super) fn write_ern_w(
        &mut self,
        register_field: u8,
        value: u16,
    ) -> Result<(), AddressingError> {
    ...
    pub(in super::super) fn read_ern_w(&self, register_field: u8) -> Result<u16, AddressingError> {
        let addr = self.read_rn_l(register_field)?;
        self.read_abs24_w(addr & 0x00ffffff)
    }

    pub(in super::super) fn write_ern_l(
        &mut self,
        register_field: u8,
        value: u32,
    ) -> Result<(), AddressingError> {
    ...
    pub(in super::super) fn read_ern_l(&self, register_field: u8) -> Result<u32, AddressingError> {
        let addr = self.read_rn_l(register_field)?;
