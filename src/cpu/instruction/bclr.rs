...
impl Cpu {
    pub(in super::super) fn bclr_rn_from_imm(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bclr_rn_from_rn(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bclr_ern(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
        ...
        return Ok(self.calc_state(StateType::I, 2)? + self.calc_state_with_addr(StateType::L, 2, access_addr)?);
    }

    pub(in super::super) fn bclr_abs(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
