...
impl Cpu {
    pub(in super::super) fn biand_rn(&mut self, opcode: u16) -> Result<usize> {
    ...
    pub(in super::super) async fn biand_ern(&mut self, opcode: u16, opcode2: u16) -> Result<usize> {
    ...
    pub(in super::super) async fn biand_abs(&mut self, opcode: u16, opcode2: u16) -> Result<usize> {
        let imm = Cpu::get_nibble_opcode(opcode2, 3)? & 7;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_biand_rn() {
        let mut cpu = Cpu::new();
        cpu.ccr = 1;
        cpu.write_rn_b(0, 0xfe).unwrap();
        cpu.bus.lock().await.memory[0..2].copy_from_slice(&[0x76, 0x80]);
        ...
        cpu.bus.lock().await.memory[0..2].copy_from_slice(&[0x76, 0xf0]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.lock().await.memory[0..2].copy_from_slice(&[0x76, 0x8f]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
    ...
    #[tokio::test]
    async fn test_biand_ern() {
        let mut cpu = Cpu::new();
        cpu.ccr = 1;
        cpu.write_abs24_b(0xffcf20, 0xfe).await.unwrap();
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.lock().await.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x76, 0x80]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.lock().await.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x76, 0xf0]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.lock().await.memory[0..4].copy_from_slice(&[0x7c, 0x70, 0x76, 0x80]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
    ...
    #[tokio::test]
    async fn test_biand_abs() {
        let mut cpu = Cpu::new();
        cpu.ccr = 1;
        cpu.write_abs24_b(0xffff12, 0xfe).await.unwrap();
        cpu.bus.lock().await.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x76, 0x80]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.lock().await.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x76, 0xf0]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
