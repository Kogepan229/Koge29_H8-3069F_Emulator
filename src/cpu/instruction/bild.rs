...
impl Cpu {
    pub(in super::super) fn bild_rn(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bild_ern(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bild_abs(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
        let imm = Cpu::get_nibble_opcode(opcode2, 3)? & 7;
...
mod tests {
    ...
    fn test_bild_rn() {
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x77, 0x80]);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x77, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x77, 0x8f]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
    ...
    fn test_bild_ern() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x77, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x77, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x70, 0x77, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
    ...
    fn test_bild_abs() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x77, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x77, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 1);
