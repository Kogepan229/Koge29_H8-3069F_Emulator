...
impl Cpu {
    pub(in super::super) fn bixor_rn(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bixor_ern(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bixor_abs(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
        let imm = Cpu::get_nibble_opcode(opcode2, 3)? & 7;
...
mod tests {
    ...
    fn test_bixor_rn() {
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x75, 0x80]);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x75, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 0);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x75, 0x8f]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 0);
    ...
    fn test_bixor_ern() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x75, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 0);
        ...
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x00, 0x75, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 0);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7c, 0x70, 0x75, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 0);
    ...
    fn test_bixor_abs() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x75, 0x80]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 0);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7e, 0x12, 0x75, 0xf0]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 6);
        assert_eq!(cpu.ccr, 0);
