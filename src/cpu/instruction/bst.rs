...
impl Cpu {
    pub(in super::super) fn bst_rn(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bst_ern(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
    ...
    pub(in super::super) fn bst_abs(&mut self, opcode: u16, opcode2: u16) -> Result<u8> {
        let imm = Cpu::get_nibble_opcode(opcode2, 3)? & 7;
...
mod tests {
    ...
    fn test_bst_rn() {
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x67, 0x00]);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x67, 0x70]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x67, 0x0f]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 1);
    ...
    fn test_bst_ern() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7d, 0x00, 0x67, 0x00]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 8);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.memory[0..4].copy_from_slice(&[0x7d, 0x00, 0x67, 0x70]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 8);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7d, 0x70, 0x67, 0x00]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 8);
        assert_eq!(cpu.ccr, 1);
    ...
    fn test_bst_abs() {
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7f, 0x12, 0x67, 0x00]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 8);
        assert_eq!(cpu.ccr, 1);
        ...
        cpu.bus.memory[0..4].copy_from_slice(&[0x7f, 0x12, 0x67, 0x70]);
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 8);
        assert_eq!(cpu.ccr, 1);
