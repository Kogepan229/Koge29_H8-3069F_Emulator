...
mod tests {
    ...
    async fn test_mov_b_disp24() {
        ...
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.write_abs24_b(0xffce0e, 0xa5).await.unwrap();
        cpu.bus.lock().await.memory[0..8].copy_from_slice(&[0x78, 0x00, 0x6a, 0x2f, 0x00, 0xff, 0xfe, 0xee]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 10);
        assert_eq!(cpu.ccr & 0b00001110, 0b00001000);
        ...
        cpu.write_rn_l(7, 0xffcf20).unwrap();
        cpu.write_abs24_b(0xffce0e, 0xa5).await.unwrap();
        cpu.bus.lock().await.memory[0..8].copy_from_slice(&[0x78, 0x70, 0x6a, 0x20, 0x00, 0xff, 0xfe, 0xee]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 10);
        assert_eq!(cpu.ccr & 0b00001110, 0b00001000);
        ...
        cpu.write_rn_b(0xf, 0xa5).unwrap();
        cpu.write_rn_l(0, 0xffcf20).unwrap();
        cpu.bus.lock().await.memory[0..8].copy_from_slice(&[0x78, 0x00, 0x6a, 0xaf, 0x00, 0xff, 0xfe, 0xee]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 10);
        assert_eq!(cpu.ccr & 0b00001110, 0b00001000);
        ...
        cpu.write_rn_b(0, 0xa5).unwrap();
        cpu.write_rn_l(7, 0xffcf20).unwrap();
        cpu.bus.lock().await.memory[0..8].copy_from_slice(&[0x78, 0x70, 0x6a, 0xa0, 0x00, 0xff, 0xfe, 0xee]);
        let opcode = cpu.fetch().await;
        let state = cpu.exec(opcode).await.unwrap();
        assert_eq!(state, 10);
        assert_eq!(cpu.ccr & 0b00001110, 0b00001000);
