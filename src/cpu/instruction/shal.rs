...
impl Cpu {
    pub(in super::super) fn shal_b(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) fn shal_w(&mut self, opcode: u16) -> Result<u8> {
        let register = Cpu::get_nibble_opcode(opcode, 4)?;
    ...
    pub(in super::super) fn shal_l(&mut self, opcode: u16) -> Result<u8> {
        let register = Cpu::get_nibble_opcode(opcode, 4)?;
...
mod tests {
    ...
    fn test_shal_l() {
        ...
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 0b00000011);
        assert_eq!(cpu.read_rn_l(0).unwrap(), 0b0101_0101_0101_0101_0101_0101_0101_0100);
        ...
        cpu.bus.memory[0..2].copy_from_slice(&[0x10, 0xb0]);
        cpu.write_rn_l(0, 0b1000_0000_0000_0000_0000_0000_0000_0000).unwrap();
        let opcode = cpu.fetch();
        let state = cpu.exec(opcode).unwrap();
        assert_eq!(state, 2);
        assert_eq!(cpu.ccr, 0b00000111);
