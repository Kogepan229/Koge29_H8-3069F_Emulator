...
impl Cpu {
    pub(in super::super) async fn subs1(&mut self, opcode: u16) -> Result<u8> {
    ...
    pub(in super::super) async fn subs2(&mut self, opcode: u16) -> Result<u8> {
        let register = Cpu::get_nibble_opcode(opcode, 4)?;
        let value = self.read_rn_l(register)?;
    ...
    pub(in super::super) async fn subs4(&mut self, opcode: u16) -> Result<u8> {
        let register = Cpu::get_nibble_opcode(opcode, 4)?;
        let value = self.read_rn_l(register)?;
